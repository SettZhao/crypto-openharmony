...
napi_value Encrypt(napi_env env, napi_callback_info info) {
    ...
    if (keyLen != 0 && memcpy_s(key, keyLen, keyPtr, keyLen) != EOK) {
        delete[] key;
        delete[] iv;
    ...
    if (ivLen != 0 && memcpy_s(iv, ivLen, ivPtr, ivLen) != EOK) {
        delete[] key;
        delete[] iv;
        EVP_CIPHER_CTX_free(ctx);
    ...
    if (algName.empty()) {
        ...
        EVP_CIPHER *cipher = const_cast<EVP_CIPHER *>(EVP_bf_cbc());
        // 完全重置 context，确保干净状态
        ...
        if (ret != 1) {
            ...
            napi_throw_error(env, nullptr,
                             ("EVP_EncryptInit_ex (set cipher) failed, code is " + std::to_string(ret)).c_str());
            return nullptr;
        }
        ...
        if (ret != 1) {
            ...
            EVP_CIPHER_CTX_free(ctx);
            napi_throw_error(env, nullptr,
                             ("EVP_EncryptInit_ex (set key/iv) failed, code is " + std::to_string(ret)).c_str());
            return nullptr;
        }
        ...
        LOGI("Context initialized: key_len=%{public}d, iv_len=%{public}d, block_size=%{public}d", actual_key_len,
             actual_iv_len, block_size);
    } else {
        delete[] key;
        delete[] iv;
        EVP_CIPHER_CTX_free(ctx);
...
extern "C" __attribute__((constructor)) void RegisterHMcryptoModule(void) { napi_module_register(&demoModule); }